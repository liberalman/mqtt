...
import com.seaofheart.app.chat.ChatDB;
...
public class VoicePlayClickListener implements View.OnClickListener {
	...
	private ProtocolMessage.CHAT_TYPE chatType;
	...
	public void playVoice(String filePath) {
		...
		if (ChatManager.getInstance().getChatOptions().getUseSpeaker()) {
			...
			if (message.direct == ProtocolMessage.DIRECT.RECEIVE) {
				try {
					if (!message.isAcked) {
						...
						// 告知对方已读这条消息
						if (chatType != ProtocolMessage.CHAT_TYPE.CHAT_GROUP)
				...
				if (!message.isListened() && iv_read_status != null && iv_read_status.getVisibility() == View.VISIBLE) {
					...
					ChatManager.getInstance().setMessageListened(message);
				}
	...
	public void onClick(View v) {
		...
		if (message.direct == ProtocolMessage.DIRECT.SEND) {
			...
			} else if (message.status == ProtocolMessage.STATUS.INPROGRESS) {
