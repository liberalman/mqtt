...
import com.seaofheart.app.chat.ContactListener;
...
import com.seaofheart.app.chat.Notifier;
import com.seaofheart.app.chat.GroupChangeListener;
...
import com.seaofheart.app.db.InviteMessgeDao;
import com.seaofheart.app.db.UserDao;
import com.seaofheart.app.domain.InviteMessage;
import com.seaofheart.app.domain.InviteMessage.InviteMesageStatus;
import com.seaofheart.app.domain.User;
import com.seaofheart.app.fx.others.LoadDataFromServer;
import com.seaofheart.app.fx.others.LoadDataFromServer.DataCallBack;
...
public class MainActivity extends BaseActivity {
    ...
    private void initView() {
        ...
        IntentFilter cmdMessageIntentFilter = new IntentFilter(ChatManager.getInstance().getCmdMessageBroadcastAction());
        cmdMessageIntentFilter.setPriority(3);
        registerReceiver(cmdMessageReceiver, cmdMessageIntentFilter);
        // 注册一个离线消息的BroadcastReceiver
        // IntentFilter offlineMessageIntentFilter = new
        // IntentFilter(ChatManager.getInstance()
        // .getOfflineMessageBroadcastAction());
        // registerReceiver(offlineMessageReceiver, offlineMessageIntentFilter);

        // setContactListener监听联系人的变化等
        ContactManager.getInstance().setContactListener(new MyContactListener());
        // 注册一个监听连接状态的listener
        ChatManager.getInstance().addConnectionListener(new MyConnectionListener());
        // 注册群聊相关的listener
        GroupManager.getInstance().addGroupChangeListener(new MyGroupChangeListener());
        // 通知sdk，UI 已经初始化完毕，注册了相应的receiver和listener, 可以接受broadcast了
        Chat.getInstance().setAppInited();
    ...
    private void notifyNewIviteMessage(InviteMessage msg) {
        saveInviteMsg(msg);
        // 提示有新消息
        Notifier.getInstance(getApplicationContext()).notifyOnNewMsg();
    ...
    private class MyConnectionListener implements EMConnectionListener {
        ...
        public void onConnected() {
            runOnUiThread(new Runnable() {
                ...
                @Override
                public void run() {
                    homefragment.errorItem.setVisibility(View.GONE); //链接成功，隐藏链接失败提示的item
                }
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    if (error == Error.USER_REMOVED) {
                        // 显示帐号已经被移除
                        showAccountRemovedDialog();
                    } else if (error == Error.CONNECTION_CONFLICT) {
                        // 显示帐号在其他设备登陆dialog
                        showConflictDialog();
                    } else {
                        homefragment.errorItem.setVisibility(View.VISIBLE); //链接失败，显示链接失败提示
    ...
    private class MyGroupChangeListener implements GroupChangeListener {
        ...
        public void onInvitationReceived(String groupId, String groupName, String inviter, String reason) {
            ...
            User user = MYApplication.getInstance().getContactList().get(inviter);
            if (user != null) {
                Message msg = Message.createReceiveMessage(ProtocolMessage.TYPE.TXT);
    ...
    public int getUnreadMsgCountTotal() {
        ...
        unreadMsgCountTotal = ChatManager.getInstance().getUnreadMsgsCount();
    ...
    public void refreshFriendsList() {
        ...
        // try {
        // usernames = ContactManager.getInstance().getContactUserNames();
