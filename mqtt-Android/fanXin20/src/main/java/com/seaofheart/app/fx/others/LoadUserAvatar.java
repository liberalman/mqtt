...
public class LoadUserAvatar {
    ...
    public Bitmap loadImage(final ImageView imageView, final String imageUrl, final ImageDownloadedCallBack imageDownloadedCallBack) {
        ...
        if (bitmap != null) {
            Log.v(TAG, "loadImag(" + imageUrl + ") image exists in memory");
            return bitmap;
        }

        // 再从文件中找
        if (fileUtil.isBitmapExists(filename)) {
            Log.v(TAG, "loadImag() image exists in file " + filename);
